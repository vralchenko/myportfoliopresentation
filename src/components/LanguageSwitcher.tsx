...
const LanguageSwitcher: React.FC<LanguageSwitcherProps> = ({ lang, setLang }) => {
    return (
        <div className="absolute top-4 left-4 md:top-6 md:right-6 md:left-auto z-[200] flex gap-1 md:gap-2 scale-75 md:scale-100 origin-top-left md:origin-top-right pointer-events-auto">
            {[
                { code: 'en', label: 'EN' },
                { code: 'de', label: 'DE' },
                { code: 'ua', label: 'UA' },
                ...
                <button
                    key={l.code}
                    onClick={(e) => { e.stopPropagation(); setLang(l.code as any); }}
